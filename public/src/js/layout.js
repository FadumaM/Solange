...
$(document).ready(function(){
 	...
 	$.fn.nextSlide = function() {
		return this.each(function() {
			...
			if(!isAnimating && $allSlides.length > 1)
			{
				...
				$nextSlide.transition({ x: windowWidth }, 0).transition({ x: 0 }, slideDuration, "easeInOutCubic", function(){
				...
				$oldCurrentSlide.transition({ x: -windowWidth }, slideDuration, "easeInOutCubic");
				...
				if(nextCaption !== $("#caption").html())
					$("#caption").transition({ opacity: 0 }, slideDuration/2, easeInOut, function(){
						...
						$("#caption").transition({ opacity: 1 }, slideDuration/2, easeInOut);
					});
 	...
 	$.fn.prevSlide = function() {
		return this.each(function() {
			...
			if(!isAnimating && $allSlides.length > 1)
			{
				...
				$prevSlide.transition({ x: -windowWidth }, 0).transition({ x: 0  }, slideDuration, "easeInOutCubic", function(){
	...
	playIntro = function(){
		...
		$("#aseat").transition({opacity: 1}, slideDuration, easeInOut);
		$("#attable").transition({opacity: 1, delay: slideDuration*3/4}, slideDuration, easeInOut);

		$("#layout").data("sliderVars").currentLeft.transition({opacity: 1, delay: slideDuration*6/4}, slideDuration, easeInOut);
		$("#layout").data("sliderVars").currentRight.transition({opacity: 1, delay: slideDuration*9/4}, slideDuration, easeInOut, function(){

			$html.removeClass("intro-animation");
			$("#aseat, #attable").removeAttr("style");
	...
	mobileIntro = function(){
		...
		$("#layout").transition({opacity: 1, delay: slideDuration*6/4}, slideDuration, easeInOut, function(){
	...
	$.fn.initHome = function(options) {
		return this.each(function() {
		    ...
		    if(hasTransitions && !hasMouse && windowWidth >= 768)
			{
				...
				homeSwipe.get("swipe").set({ direction: Hammer.DIRECTION_ALL, preventDefault: true });
	...
	$.fn.prevLeft = function() {
		return this.each(function() {
			...
			if(!isAnimatingLeft && $allLeft.length > 1)
			{
				...
				$prevLeft.transition({ y: -windowHeight }, 0).transition({ y: 0 }, slideDuration, "easeInOutCubic", function(){
				...
				$oldCurrentLeft.transition({ y: windowHeight }, slideDuration, "easeInOutCubic");
			}
		});
	};
 	...
 	$.fn.nextLeft = function() {
		return this.each(function() {
			...
			if(!isAnimatingLeft && $allLeft.length > 1)
			{
				...
				$nextLeft.transition({ y: windowHeight }, 0).transition({ y: 0 }, slideDuration, "easeInOutCubic", function(){
	...
	$.fn.nextRight = function() {
		return this.each(function() {
			...
			if(!isAnimatingRight && $allRight.length > 1)
			{
				...
				$nextRight.transition({ y: -windowHeight }, 0).transition({ y: 0 }, slideDuration, "easeInOutCubic", function(){
				...
				$oldCurrentRight.transition({ y: windowHeight }, slideDuration, "easeInOutCubic");
			}
		});
	};
 	...
 	$.fn.prevRight = function() {
		return this.each(function() {
			...
			if(!isAnimatingRight && $allRight.length > 1)
			{
				...
				$prevRight.transition({ y: windowHeight }, 0).transition({ y: 0 }, slideDuration, "easeInOutCubic", function(){
				...
				if(windowWidth >= 768)
				{
					...
					$slider.data("sliderVars").allSlides.each(function(){
						...
						if(!$body.is(".home"))
						{
							...
							{
								$slideObj.transition({ x: windowWidth }, 0);
							}
			...
			if($slider.is(".initialised"))
			{
				...
				if(windowWidth >= 768)
				{
					...
					$slider.data("sliderVars").allLeft.each(function(){
						...
						if($slideObj.is(".current"))
						{
							$slideObj.transition({ y: 0 }, 0);
						}
						else if($slideObj.is(".next-left"))
						{
							$slideObj.transition({ y: windowHeight }, 0);
						}
    ...
    if(hasMouse)
    {
		$(document).on("mouseenter", "#aseat, #attable", function(){
			$(this).addClass("hovered");
		});

		$(document).on("mouseleave", "#aseat, #attable", function(){
	...
	closeAbout = function(){
	    ...
	    $left.transition({ x: -Math.ceil(windowWidth/2) }, 0).transition({ x: 0, opacity: 1 }, slideDuration, "easeInOutQuint");
	    $right.transition({ x:  Math.ceil(windowWidth/2) }, 0).transition({ x: 0, opacity: 1 }, slideDuration, "easeInOutQuint");

	    $body.removeClass("force-white");

	    $("#about").attr("style", "visibility: visible");

	    $("#left-bg").transition({ x:  Math.ceil(windowWidth/2) }, slideDuration*9/10, "easeInOutQuint");
	    $("#right-bg").transition({ x: -Math.ceil(windowWidth/2) }, slideDuration*9/10, "easeInOutQuint");

		$("#about-text").transition({ opacity: 0 }, opacityDuration, easeInOut);

	    $("#aseat").transition({ x: Math.ceil(windowWidth/2 - $("#aseat").width() - 39) }, slideDuration, "easeInOutQuint");
	    $("#attable").transition({ x: Math.ceil(-windowWidth/2 + $("#attable").width() + 39) }, slideDuration, "easeInOutQuint");

	    setTimeout(function(){
		    $body.removeClass("about-open");
		    $("#left-bg, #right-bg, #aseat, #attable, #about-text, #about").removeAttr("style");
	...
	closeMobileAbout = function(){
		$("#about-text, #about-close").transition({ opacity: 0 }, opacityDuration, easeInOut, function(){
			$("#about").attr("style", "visibility: hidden");

			$body.removeClass("force-white");

			$("#aseat").transition({ x: 50 }, opacityDuration, "easeInOutQuint");
			$("#attable").transition({ x: -50 }, opacityDuration, "easeInOutQuint");

			$("#layout").attr("style", "visibility: visible; position: static;").transition({ opacity: 1 }, opacityDuration, easeInOut, function(){
				$body.removeClass("about-open");
			    $("#layout, #about, #about-text, #aseat, #attable").removeAttr("style");
			    isAnimating = false;
			});
		});
	};

    $(document).on("click", "#aseat, #attable", function(){

		    if(!isAnimating)
		    {
		    	$("#aseat, #attable").removeClass("hovered");
				isAnimating = true;
				...
				if(windowWidth >= 768)
				{
				    if(!$body.is(".about-open"))
				    {
					    ...
					    $left.transition({ x: -Math.floor(windowWidth/2), opacity: 0 }, slideDuration, "easeInOutQuint");
					    $right.transition({ x: Math.floor(windowWidth/2), opacity: 0 }, slideDuration, "easeInOutQuint");

					    $("#about").attr("style", "visibility: visible");

					    $("#left-bg").transition({ x: -Math.floor(windowWidth/2) }, slideDuration, "easeInOutQuint");
					    $("#right-bg").transition({ x:  Math.floor(windowWidth/2) }, slideDuration, "easeInOutQuint");

						$("#about-text").transition({ opacity: 1, delay: slideDuration/2 }, opacityDuration, easeInOut);

					    $("#aseat").transition({ x: Math.floor(-windowWidth/2 + $("#aseat").width() + 39) }, slideDuration*9/10, "easeInOutQuint");
					    $("#attable").transition({ x:  Math.floor(windowWidth/2 - $("#attable").width() - 39) }, slideDuration*9/10, "easeInOutQuint");
			    ...
			    else
			    {
					if(!$body.is(".about-open"))
					{
						...
						$("#layout").transition({ opacity: 0 }, opacityDuration, easeInOut, function(){

							$("#about").attr("style", "visibility: visible");
							$("#about-text, #about-close").transition({ opacity: 1 }, opacityDuration, easeInOut, function(){
								$body.addClass("about-open");
							    $("#layout, #about, #about-text, #about-close, #aseat, #attable").removeAttr("style");
							    isAnimating = false;
							});
						});
					}
	...
	if(Modernizr.history && windowWidth >= 768)
	{
		...
		projectLoad = function(targetHref, targetTitle, halfID, currentIndex, targetIndex){
			...
			$otherHalf.transition({opacity: 0}, slideDuration/2, easeInOut);

			$("#aseat-mask").transition({ x: $("#aseat-mask").width() }, slideDuration*3/4, "easeInOutQuint");
			$("#attable-mask").transition({ x: -$("#attable-mask").width() }, slideDuration*3/4, "easeInOutQuint");
			...
			$('<div></div>').load(targetHref + "?ajax=1 #project", function(response, status){
			    ...
			    if(status !== "error")
			    {
				    ...
				    $("#aseat-mask, #attable-mask").removeAttr("style");

				    imageReady = false;

					$("#slider").initSlider({ slideIndex: currentIndex, targetIndex: targetIndex, isRight: isRight });
					...
					showSlider = function(){
						...
						if(imageReady)
						{
							$("#project").transition({opacity: 1}, slideDuration/2, easeInOut, function(){
								...
								if(currentIndex !== targetIndex)
								{
									...
									if(currentIndex !== targetIndex)
										setTimeout(function(){
											$("#thumbs, #close").transition({opacity: 1}, slideDuration/2, easeInOut).promise().done(function(){
					...
					setTimeout(function(){
						$("#clone").transition({ x: $targetImg.offset().left - $("#clone").offset().left, y: $targetImg.offset().top - $("#clone").offset().top, scale: $targetImg.height()/$("#clone").height() }, slideDuration*3/4, "easeInOutQuint", function(){
							showSlider();
						});
					}, 50);
				}
		...
		projectClose = function(targetHref, targetTitle, halfID, currentIndex, targetIndex){
			...
			setTimeout(function(){
				...
				$("#project").transition({opacity: 0}, slideDuration/2, easeInOut);

				$("#clone").transition({ x: $targetImg.offset().left - $("#clone").offset().left, y: $targetImg.offset().top - $("#clone").offset().top, scale: $targetImg.height()/$("#clone").height() }, slideDuration*3/4, "easeInOutQuint", function(){

					updateHistory(targetHref, targetTitle);

					$("#aseat, #attable").attr("style", "visibility: visible; opacity: 1");

					$("#aseat-mask").transition({ x: -$("#aseat-mask").width() }, slideDuration*3/4, "easeInOutQuint");
					$("#attable-mask").transition({ x: $("#attable-mask").width() }, slideDuration*3/4, "easeInOutQuint", function(){
					...
					$otherHalf.attr("style", "opacity: 0").transition({opacity: 1}, slideDuration/2, easeInOut);
				});
			}, extraDelay);
		};
	...
	if(windowWidth < 768)
	{
		...
		$(document).on("click", "#close", function(e){
			...
			if(document.referrer.indexOf("aseatattable") != -1)
			{
				e.preventDefault();
				window.history.back();
			}
